-- Initial multi-tenant schema for the equipment-contractor SaaS MVP.
-- Tenancy model: every business table carries company_id and is locked down
-- with RLS so a signed-in user only ever sees rows from their own company.
-- Auth: auth.users (Supabase Auth) is the source of identity; public.users
...
-- ---------------------------------------------------------------------------
-- quote_items: item/unit-price patterns accumulated per company, seeded from
-- past-quote imports and reused as button-selectable presets when building
-- a new quote.
-- ---------------------------------------------------------------------------
...
create table if not exists public.quotes (
  ...
  -- Set when an AI-generated draft is accepted;金額確定は必ず人の操作を経る
  -- (spec requirement) so this is informational only, never a bypass.
...
create table if not exists public.past_quote_imports (
  ...
  -- Raw AI extraction output, kept for auditing/re-review.
  parsed_result jsonb,
...
-- companies: members can read their own company; writes go through
-- server-side routes using the service role key (e.g. onboarding, plan
-- changes), so no insert/update policy is granted to regular users here.
create policy "companies_select_own" on public.companies
  for select using (id = public.current_company_id());

-- users: members can see/manage profiles within their own company.
...
-- quotes: full CRUD scoped to company_id (dashboard toggles between "mine"
-- and "company-wide" purely by filtering on owner_id client-side/query-side,
-- not via RLS, per spec: view-only split, not a permission split).
