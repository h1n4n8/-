...
import { FolderOpen, Calendar, Users, FileText, TrendingUp, Clock, AlertCircle } from "lucide-react";
    ...
    <AppShell>
      <div className="px-6 py-8 max-w-5xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-800 mt-0.5">こんにちは、{user.name}さん</h1>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-6">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-50">
            ...
            <button onClick={() => router.push("/projects")} className="text-sm text-blue-600 hover:underline">すべて見る</button>
          </div>
          <div className="divide-y divide-gray-50">
            {recentProjects.map((p) => (
              <div key={p.id} className="flex items-center justify-between px-6 py-4 hover:bg-gray-50 cursor-pointer transition-colors">
                <div>
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl px-5 py-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-amber-600 mt-0.5">「〇〇マンション空調工事」の工期終了まで5日です</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 mt-6">
          {[
            ...
            <button key={label} onClick={() => router.push(href)} className="flex items-center gap-3 px-5 py-4 rounded-2xl border border-gray-100 bg-white hover:shadow-sm transition-shadow">
